<section class="grid grid-cols-2 gap-8 mt-16">
  <div class="rounded-xl bg-[var(--bg2)] border border-[var(--border)] p-8 grid grid-cols-3 gap-4">
    ...
    <div class="flex flex-col col-span-1 gap-4 *:w-full *:justify-center *:p-4  *:rounded-lg *:flex *:items-center *:gap-2 *:h-full">
      <div class="bg-green-600" title="Health Points">
        ...
        <?= $this->player->getHeroCharacter()->getHealthPoints() ?>/<?= $this->player->getHeroCharacter()->getMaxHealthPoints() ?>
      </div>
    ...
    <div class="col-span-full flex flex-col gap-4">
      ...
      <form class="grid grid-cols-2 gap-4 *:w-full *:p-4 *:rounded-lg *:flex *:items-center *:justify-between" method="POST">
        ...
        ?>
          <button title="<?= $title ?>" class="<?= $color ?> disabled:opacity-50" type="submit" name="action" value="<?= $action->getName() ?>" <?= $disabled ?>>
  ...
  <div class="rounded-xl h-fit bg-[var(--bg2)] border border-[var(--border)] p-8 grid grid-cols-3 gap-4">
    ...
    <div class="col-span-full aspect-video">
      <img class="rounded-lg w-full h-full object-cover" src="https://cataas.com/cat" alt="">
    </div>
    <div class="flex flex-col col-span-1 gap-4 *:w-full *:justify-center *:p-4  *:rounded-lg *:flex *:items-center *:gap-2 *:h-full">
